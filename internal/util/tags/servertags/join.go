...
func Join(tags []orcv1alpha1.ServerTag) string {
...
func ReconcileTags[orcObjectPT, osResourceT any](
	computeClient osclients.ComputeClient,
	resourceID string,
	specTags []orcv1alpha1.ServerTag,
	observedTags []string,
	...
	return func(ctx context.Context, _ orcObjectPT, _ *osResourceT) progress.ReconcileStatus {
		...
		if !specTagSet.Equal(observedTagSet) {
			opts := tags.ReplaceAllOpts{Tags: specTagSet.SortedList()}
