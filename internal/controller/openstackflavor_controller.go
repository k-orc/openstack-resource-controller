...
func (r *OpenStackFlavorReconciler) reconcile(ctx context.Context, computeClient *gophercloud.ServiceClient, resource, statusPatchResource *openstackv1.OpenStackFlavor) (ctrl.Result, error) {
	...
	if openstackID := coalesce(resource.Spec.ID, resource.Status.Resource.ID); openstackID != "" {
		...
		flavor, err = r.flavorFind(log.IntoContext(ctx, logger), computeClient, resource, createOpts)
		if err != nil {
...
func (r *OpenStackFlavorReconciler) flavorFind(ctx context.Context, computeClient *gophercloud.ServiceClient, resource client.Object, createOpts flavors.CreateOpts) (*flavors.Flavor, error) {
