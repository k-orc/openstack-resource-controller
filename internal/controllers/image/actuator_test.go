...
func TestHandleMinDiskUpdate(t *testing.T) {
	ptrToInt32 := ptr.To[int32]

	testCases := []struct {
		name          string
		properties    *orcv1alpha1.ImageProperties
		existingValue int
		expectChange  bool
	}{
		{
			name:          "Identical",
			properties:    &orcv1alpha1.ImageProperties{MinDiskGB: ptrToInt32(20)},
			existingValue: 20,
			expectChange:  false,
		},
		{
			name:          "Different",
			properties:    &orcv1alpha1.ImageProperties{MinDiskGB: ptrToInt32(30)},
			existingValue: 20,
			expectChange:  true,
		},
		{
			name:          "Setting from zero",
			properties:    &orcv1alpha1.ImageProperties{MinDiskGB: ptrToInt32(10)},
			existingValue: 0,
			expectChange:  true,
		},
		{
			name:          "Field not specified in spec",
			properties:    &orcv1alpha1.ImageProperties{},
			existingValue: 20,
			expectChange:  false,
		},
		{
			name:          "Properties block is nil in spec",
			properties:    nil,
			existingValue: 20,
			expectChange:  false,
		},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			osResource := &images.Image{MinDiskGigabytes: tt.existingValue}
			updateOpts := images.UpdateOpts{}
...
func TestHandleMinRamUpdate(t *testing.T) {
	ptrToInt32 := ptr.To[int32]
	...
	testCases := []struct {
		...
		{
			name:          "Identical",
			properties:    &orcv1alpha1.ImageProperties{MinMemoryMB: ptrToInt32(1024)},
			existingValue: 1024,
			expectChange:  false,
		},
		{
			name:          "Different",
			properties:    &orcv1alpha1.ImageProperties{MinMemoryMB: ptrToInt32(2048)},
			existingValue: 1024,
			expectChange:  true,
		},
		{
			name:          "Setting from zero",
			properties:    &orcv1alpha1.ImageProperties{MinMemoryMB: ptrToInt32(512)},
			existingValue: 0,
			expectChange:  true,
		},
		{
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			osResource := &images.Image{MinRAMMegabytes: tt.existingValue}
			updateOpts := images.UpdateOpts{}
