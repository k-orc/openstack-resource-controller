...
func (actuator imageActuator) UpdateResource(ctx context.Context, obj orcObjectPT, osResource *osResourceT) progress.ReconcileStatus {
	...
	updateOpts = handleMinDiskUpdate(updateOpts, resource, osResource)
	updateOpts = handleMinRamUpdate(updateOpts, resource, osResource)
...
func handleProtectedUpdate(updateOpts images.UpdateOpts, resource *resourceSpecT, osResource *osResourceT) images.UpdateOpts {
	desiredProtected := resource.Protected

	if desiredProtected == nil {
		return updateOpts
	}

	protectedValue := *desiredProtected
...
func handleMinDiskUpdate(updateOpts images.UpdateOpts, resource *resourceSpecT, osResource *osResourceT) images.UpdateOpts {
	if resource.Properties == nil || resource.Properties.MinDiskGB == nil {
		return updateOpts
	}

	desiredMinDisk := int(*resource.Properties.MinDiskGB)

	if osResource.MinDiskGigabytes != desiredMinDisk {
		patch := images.ReplaceImageMinDisk{
			NewMinDisk: desiredMinDisk,
		}
		updateOpts = append(updateOpts, patch)
	}
...
func handleMinRamUpdate(updateOpts images.UpdateOpts, resource *resourceSpecT, osResource *osResourceT) images.UpdateOpts {
	if resource.Properties == nil || resource.Properties.MinMemoryMB == nil {
		return updateOpts
	}

	desiredMinRam := int(*resource.Properties.MinMemoryMB)

	if osResource.MinRAMMegabytes != desiredMinRam {
		patch := images.ReplaceImageMinRam{
			NewMinRam: desiredMinRam,
		}
		updateOpts = append(updateOpts, patch)
	}
