...
func (volumetypeActuator) GetResourceID(osResource *volumetypes.VolumeType) string {
	return osResource.ID
}

func (actuator volumetypeActuator) GetOSResourceByID(ctx context.Context, id string) (*volumetypes.VolumeType, progress.ReconcileStatus) {
	volumetype, err := actuator.osClient.GetVolumeType(ctx, id)
	if err != nil {
...
func (actuator volumetypeActuator) ListOSResourcesForAdoption(ctx context.Context, orcObject orcObjectPT) (iter.Seq2[*volumetypes.VolumeType, error], bool) {
	...
	if resourceSpec.IsPublic != nil {
		filters = append(filters, func(f *volumetypes.VolumeType) bool {
			return f.IsPublic == *resourceSpec.IsPublic
		})
	}
...
func (actuator volumetypeActuator) ListOSResourcesForImport(ctx context.Context, obj orcObjectPT, filter filterT) (iter.Seq2[*osResourceT, error], progress.ReconcileStatus) {
	...
	if filter.Name != nil {
		filters = append(filters, func(f *volumetypes.VolumeType) bool { return f.Name == string(*filter.Name) })
	}

	if filter.Description != nil {
		filters = append(filters, func(f *volumetypes.VolumeType) bool { return f.Description == *filter.Description })
	}

	if filter.IsPublic != nil {
		filters = append(filters, func(f *volumetypes.VolumeType) bool { return f.IsPublic == *filter.IsPublic })
	}

	return actuator.listOSResources(ctx, filters, &volumetypes.ListOpts{}), nil
}
...
func (actuator volumetypeActuator) CreateResource(ctx context.Context, obj orcObjectPT) (*volumetypes.VolumeType, progress.ReconcileStatus) {
...
func (actuator volumetypeActuator) DeleteResource(ctx context.Context, _ orcObjectPT, volumetype *volumetypes.VolumeType) progress.ReconcileStatus {
	return progress.WrapError(actuator.osClient.DeleteVolumeType(ctx, volumetype.ID))
}
...
func needsUpdate(updateOpts volumetypes.UpdateOpts) (bool, error) {
	...
	volumetypeUpdateMap, ok := updateOptsMap["volume_type"].(map[string]any)
	if !ok {
		volumetypeUpdateMap = make(map[string]any)
	}

	return len(volumetypeUpdateMap) > 0, nil
}
