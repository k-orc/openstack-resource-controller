...
func (c *Controller[
	...
	defer func() {
		...
		if errors.As(err, &terminalError) {
			log.V(logging.Status).Info("not scheduling further reconciles for terminal error", "err", err.Error())
			err = nil
		}
	}()
	...
	if len(progressStatus) > 0 {
		log.V(logging.Info).Info("Waiting on events before creation")
...
func (c *Controller[
	...
	log := ctrl.LoggerFrom(ctx)
	log.V(logging.Info).Info("Reconciling OpenStack resource delete")
