...
func GetOrCreateOSResource[
	...
	if resourceID := objAdapter.GetStatusID(); resourceID != nil {
		...
		if osResource != nil {
			log.V(logging.Info).Info("Got existing OpenStack resource", "ID", actuator.GetResourceID(osResource))
		}
	...
	if resourceID := objAdapter.GetImportID(); resourceID != nil {
		...
		if osResource != nil {
			log.V(logging.Status).Info("Imported existing OpenStack resource by ID", "ID", actuator.GetResourceID(osResource))
		}
		return nil, osResource, err
	}
...
func DeleteResource[
	...
	if managementPolicy == orcv1alpha1.ManagementPolicyUnmanaged || managedOptions.GetOnDelete() == orcv1alpha1.OnDeleteDetach {
		...
		log.V(logging.Status).Info("Not deleting OpenStack resource due to policy", logPolicy...)
	...
	if osResource == nil {
		log.V(logging.Info).Info("Resource is no longer observed")
