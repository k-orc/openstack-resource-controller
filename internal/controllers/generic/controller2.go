...
) Controller2[orcObjectPT, orcObjectT, resourceSpecT, filterT, objectApplyPT, statusApplyPT, statusApplyT, osResourceT] {
	return Controller2[orcObjectPT, orcObjectT, resourceSpecT, filterT, objectApplyPT, statusApplyPT, statusApplyT, osResourceT]{
		name:          name,
		client:        k8sClient,
...
func (c *Controller2[_, _, _, _, _, _, _, _]) GetName() string {
	return c.name
}

func (c *Controller2[_, _, _, _, _, _, _, _]) GetK8sClient() client.Client {
	return c.client
}

func (c *Controller2[_, _, _, _, _, _, _, _]) GetScopeFactory() scope.Factory {
...
func (c *Controller2[
	...
	waitEvents, osResource, err = GetOrCreateOSResource2(ctx, log, c, objAdapter, actuator)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (c *Controller2[
	...
	deleted, waitEvents, osResource, err = DeleteResource2(ctx, log, c, objAdapter, actuator)
