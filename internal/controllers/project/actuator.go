...
func (actuator projectActuator) updateResource(ctx context.Context, obj orcObjectPT, osResource *osResourceT) progress.ReconcileStatus {
	...
	// FIXME(dkokkino): Unlike the networkClient, which supports tag updates for controllers
	// like subnets, ports, networks, and security groups, the identityClient doesn't support
	// tag updates via the ReconcileTags method. Using a custom handler to update tags as a workaround for now.
