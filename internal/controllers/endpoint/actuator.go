...
func (actuator endpointActuator) ListOSResourcesForImport(ctx context.Context, obj orcObjectPT, filter filterT) (iter.Seq2[*osResourceT, error], progress.ReconcileStatus) {
	...
	if filter.ServiceRef != nil {
		serviceKey := client.ObjectKey{Name: string(*filter.ServiceRef), Namespace: obj.Namespace}
		if err := actuator.k8sClient.Get(ctx, serviceKey, service); err != nil {
			if apierrors.IsNotFound(err) {
				reconcileStatus = reconcileStatus.WithReconcileStatus(
					progress.WaitingOnObject("Service", serviceKey.Name, progress.WaitingOnCreation))
			} else {
				reconcileStatus = reconcileStatus.WithReconcileStatus(
					progress.WrapError(fmt.Errorf("fetching service %s: %w", serviceKey.Name, err)))
			}
		} else {
			if !orcv1alpha1.IsAvailable(service) || service.Status.ID == nil {
				reconcileStatus = reconcileStatus.WithReconcileStatus(
					progress.WaitingOnObject("Service", serviceKey.Name, progress.WaitingOnReady))
			}
		}
	}
	...
	listOpts := endpoints.ListOpts{
		...
		Service:  ptr.Deref(service.Status.ID, ""),
		// TODO(scaffolding): Add more import filters
	}

	return actuator.osClient.ListEndpoints(ctx, listOpts), nil
}
