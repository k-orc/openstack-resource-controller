...
func TestNeedsUpdate(t *testing.T) {
	testCases := []struct {
		...
		{
			name:         "Updated opts",
			updateOpts:   containers.UpdateOpts{Name: ptr.To("updated")},
			expectChange: true,
		},
	}
...
func TestHandleNameUpdate(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.SwiftContainer{}
			resource.Name = "object-name"
			resource.Spec = orcv1alpha1.SwiftContainerSpec{
				Resource: &orcv1alpha1.SwiftContainerResourceSpec{Name: tt.newValue},
			}
...
func TestHandleDescriptionUpdate(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.SwiftContainerResourceSpec{Description: tt.newValue}
