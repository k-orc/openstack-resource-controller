...
func TestNeedsUpdate(t *testing.T) {
	testCases := []struct {
		...
		{
			name:         "Updated opts",
			updateOpts:   sharenetworks.UpdateOpts{Name: ptr.To("updated")},
			expectChange: true,
		},
	}
...
func TestHandleNameUpdate(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.ShareNetwork{}
			resource.Name = "object-name"
			resource.Spec = orcv1alpha1.ShareNetworkSpec{
				Resource: &orcv1alpha1.ShareNetworkResourceSpec{Name: tt.newValue},
			}
...
func TestHandleDescriptionUpdate(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.ShareNetworkResourceSpec{Description: tt.newValue}
