...
independently. The jitter mechanism (±10%) ensures they are not all
re-reconciled at exactly the same instant, which would cause a thundering-herd
effect. Because the ±10% jitter applied to a 10s period produces scheduling
spread of up to ±1s, we check that all three networks successfully re-synced
