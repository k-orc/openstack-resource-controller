...
func (subnetStatusWriter) ApplyResourceStatus(log logr.Logger, osResource *osResourceT, statusApply statusApplyPT) {
	...
	if osResource.Name != "" {
		status.WithName(osResource.Name)
	}
	if osResource.Description != "" {
		status.WithDescription(osResource.Description)
	}
	if osResource.IPVersion > 0 {
		status.WithIPVersion(int32(osResource.IPVersion))
	}
	if osResource.CIDR != "" {
		status.WithCIDR(osResource.CIDR)
	}
	if osResource.GatewayIP != "" {
		status.WithGatewayIP(osResource.GatewayIP)
	}
	if osResource.NetworkID != "" {
		status.WithNetworkID(osResource.NetworkID)
	}
	if osResource.ProjectID != "" {
		status.WithProjectID(osResource.ProjectID)
	}
