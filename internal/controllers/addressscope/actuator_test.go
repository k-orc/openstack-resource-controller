...
func TestNeedsUpdate(t *testing.T) {
	testCases := []struct {
		...
		{
			name:         "Updated opts",
			updateOpts:   addressscopes.UpdateOpts{Name: ptr.To("updated")},
			expectChange: true,
		},
	}
...
func TestHandleNameUpdate(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.AddressScope{}
			resource.Name = "object-name"
			resource.Spec = orcv1alpha1.AddressScopeSpec{
				Resource: &orcv1alpha1.AddressScopeResourceSpec{Name: tt.newValue},
			}
