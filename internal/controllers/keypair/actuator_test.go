...
func (m mockKeyPairClient) GetKeyPair(_ context.Context, _ string, _ string) (*keypairs.KeyPair, error) {
...
func (m mockKeyPairClient) DeleteKeyPair(_ context.Context, _ string, _ string) error {
...
func TestListOSResourcesForImport(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			keypairs: []keypairs.KeyPair{
				{Name: "test-keypair", Fingerprint: "aa:bb:cc", UserID: "user1"},
				{Name: "other-keypair", Fingerprint: "dd:ee:ff", UserID: "user1"},
			},
			filter: orcv1alpha1.KeyPairFilter{
		...
		},
		{
			...
			keypairs: []keypairs.KeyPair{
				{Name: "keypair1", Fingerprint: "aa:bb:cc", UserID: "user1"},
				{Name: "keypair2", Fingerprint: "dd:ee:ff", UserID: "user1"},
				{Name: "keypair3", Fingerprint: "11:22:33", UserID: "user1"},
			},
			filter: orcv1alpha1.KeyPairFilter{},
			checks: checks(
				noError,
		...
		},
		{
			...
			keypairs: []keypairs.KeyPair{
				{Name: "keypair1", Fingerprint: "aa:bb:cc", UserID: "user1"},
				{Name: "keypair2", Fingerprint: "dd:ee:ff", UserID: "user2"},
			},
			filter: orcv1alpha1.KeyPairFilter{
				UserID: ptr.To("user1"),
			},
			checks: checks(
				noError,
				// Note: Server-side filtering by userID happens in OpenStack API,
				// so this mock would need to implement that to test properly
				findsN(2), // Mock returns all
			),
		},
	}
...
func TestListOSResourcesForAdoption(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			keypairs: []keypairs.KeyPair{
				{Name: "my-keypair", Fingerprint: "aa:bb:cc", UserID: "user1"},
				{Name: "other-keypair", Fingerprint: "dd:ee:ff", UserID: "user1"},
			},
