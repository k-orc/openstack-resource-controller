...
func TestNeedsUpdate(t *testing.T) {
	testCases := []struct {
		...
		{
			name:         "Updated opts",
			updateOpts:   keypairs.UpdateOpts{Name: ptr.To("updated")},
			expectChange: true,
		},
	}
...
func TestHandleNameUpdate(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.KeyPair{}
			resource.Name = "object-name"
			resource.Spec = orcv1alpha1.KeyPairSpec{
				Resource: &orcv1alpha1.KeyPairResourceSpec{Name: tt.newValue},
			}
...
func TestHandleDescriptionUpdate(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.KeyPairResourceSpec{Description: tt.newValue}
