...
func (actuator userActuator) CreateResource(ctx context.Context, obj orcObjectPT) (*osResourceT, progress.ReconcileStatus) {
	...
	createOpts := users.CreateOpts{
		...
		DefaultProjectID: string(ptr.Deref(resource.DefaultProjectID, "")),
		DomainID:         domainID,
		Password:         string(ptr.Deref(resource.Password, "")),
...
func handlePasswordUpdate(updateOpts *users.UpdateOpts, resource *resourceSpecT) {
	if resource.Password != nil {
		updateOpts.Password = string(ptr.Deref(resource.Password, ""))
	}
}
