...
| `userID` _string_ | userID to filter by. Only admin users can use this. |  | MaxLength: 255 <br /> |
...
| `publicKey` _string_ | publicKey is the public key to import. If not specified, a new Keypair<br />will be generated by OpenStack and the private key will be stored in<br />the secret specified by privateKeySecretRef. |  | MaxLength: 16384 <br /> |
| `privateKeySecretRef` _[SecretReference](#secretreference)_ | privateKeySecretRef references a secret where the private key will be<br />stored when a new Keypair is generated. This is required when publicKey<br />is not specified (i.e., when generating a new Keypair).<br />The private key will be stored in the secret under the key "private_key". |  |  |
| `userID` _string_ | userID is the user ID for the Keypair. Only admin users can specify<br />this to create/import Keypairs for other users. Requires microversion 2.10+. |  | MaxLength: 255 <br /> |
...
| `userID` _string_ | userID is the user ID that owns this Keypair |  | MaxLength: 255 <br /> |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `name` _string_ | name is the name of the secret |  | MaxLength: 253 <br />MinLength: 1 <br /> |
| `key` _string_ | key is the key in the secret where data is stored. Defaults to the field name. |  | MaxLength: 253 <br /> |
