...
This document describes the high-level design philosophy and architectural decisions that shape ORC. Understanding these principles will help you make consistent design choices when contributing to the project.
...
- we reserve the right to rename some fields (for instance, ports `Fixed IPs` changed to `IP Addresses`)
...
We allow name reuse of ORC objects. This is safe, thanks to ORC's dependency management.
