...
type templateFields struct {
	...
	Name       string
	NameLower  string
	// NamePlural is the lower-cased plural form of the resource name, used to
	// reference the CRD base file generated by controller-gen. It defaults to
	// NameLower + "s". Resources whose name does not pluralize by simply
	// appending "s" (e.g. words ending in "y", "s", "x", "ch", "sh") MUST set
	// this explicitly to match the plural that controller-gen derives
	// (e.g. QosPolicy -> "qospolicies").
	NamePlural             string
	IsNotNamed             bool
...
func addDefaults(resources []templateFields) {
	for i := range resources {
		...
		if resource.NamePlural == "" {
			resource.NamePlural = resource.NameLower + "s"
		}
	}
}
