...
func main() {
	...
	funcMap := template.FuncMap{"plural": pluralize}
	crdKustomizationTemplate := template.Must(template.New("crd-kustomization").Funcs(funcMap).Parse(crd_kustomization_template))
