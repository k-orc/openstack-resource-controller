...
// VolumeTypeInformer provides access to a shared informer and lister for
// VolumeTypes.
...
type volumeTypeInformer struct {
	factory          internalinterfaces.SharedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
	namespace        string
}

// NewVolumeTypeInformer constructs a new informer for VolumeType type.
...
func NewVolumeTypeInformer(client clientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredVolumeTypeInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredVolumeTypeInformer constructs a new informer for VolumeType type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredVolumeTypeInformer(client clientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		...
		&v3apiv1alpha1.VolumeType{},
		resyncPeriod,
		indexers,
	)
}

func (f *volumeTypeInformer) defaultInformer(client clientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredVolumeTypeInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *volumeTypeInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&v3apiv1alpha1.VolumeType{}, f.defaultInformer)
}

func (f *volumeTypeInformer) Lister() apiv1alpha1.VolumeTypeLister {
	return apiv1alpha1.NewVolumeTypeLister(f.Informer().GetIndexer())
}
