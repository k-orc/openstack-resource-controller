...
// Router constructs a declarative configuration of the Router type for use with
// apply.
...
// ExtractRouter extracts the applied configuration owned by fieldManager from
// router. If no managedFields are found in router for fieldManager, a
// RouterApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// router must be a unmodified Router API object that was retrieved from the Kubernetes API.
// ExtractRouter provides a way to perform a extract/modify-in-place/apply workflow.
...
func ExtractRouter(router *apiv1alpha1.Router, fieldManager string) (*RouterApplyConfiguration, error) {
	return extractRouter(router, fieldManager, "")
}

// ExtractRouterStatus is the same as ExtractRouter except
// that it extracts the status subresource applied configuration.
// Experimental!
func ExtractRouterStatus(router *apiv1alpha1.Router, fieldManager string) (*RouterApplyConfiguration, error) {
	return extractRouter(router, fieldManager, "status")
}

func extractRouter(router *apiv1alpha1.Router, fieldManager string, subresource string) (*RouterApplyConfiguration, error) {
	b := &RouterApplyConfiguration{}
	err := managedfields.ExtractInto(router, internal.Parser().Type("com.github.k-orc.openstack-resource-controller.api.v1alpha1.Router"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(router.Name)
	b.WithNamespace(router.Namespace)
...
func (b *RouterApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
