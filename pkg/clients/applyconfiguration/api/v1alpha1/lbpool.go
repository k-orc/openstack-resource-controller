...
// ExtractLBPool extracts the applied configuration owned by fieldManager from
// lBPool. If no managedFields are found in lBPool for fieldManager, a
// LBPoolApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// lBPool must be a unmodified LBPool API object that was retrieved from the Kubernetes API.
// ExtractLBPool provides a way to perform a extract/modify-in-place/apply workflow.
// Note that an extracted apply configuration will contain fewer fields than what the fieldManager previously
// applied if another fieldManager has updated or force applied any of the previously applied fields.
// Experimental!
func ExtractLBPool(lBPool *apiv1alpha1.LBPool, fieldManager string) (*LBPoolApplyConfiguration, error) {
	return extractLBPool(lBPool, fieldManager, "")
}

// ExtractLBPoolStatus is the same as ExtractLBPool except
// that it extracts the status subresource applied configuration.
// Experimental!
func ExtractLBPoolStatus(lBPool *apiv1alpha1.LBPool, fieldManager string) (*LBPoolApplyConfiguration, error) {
	return extractLBPool(lBPool, fieldManager, "status")
}

func extractLBPool(lBPool *apiv1alpha1.LBPool, fieldManager string, subresource string) (*LBPoolApplyConfiguration, error) {
	b := &LBPoolApplyConfiguration{}
	err := managedfields.ExtractInto(lBPool, internal.Parser().Type("com.github.k-orc.openstack-resource-controller.v2.api.v1alpha1.LBPool"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(lBPool.Name)
	b.WithNamespace(lBPool.Namespace)
