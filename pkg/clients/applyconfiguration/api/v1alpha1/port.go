...
// ExtractPort extracts the applied configuration owned by fieldManager from
// port. If no managedFields are found in port for fieldManager, a
// PortApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// port must be a unmodified Port API object that was retrieved from the Kubernetes API.
// ExtractPort provides a way to perform a extract/modify-in-place/apply workflow.
...
func ExtractPort(port *apiv1alpha1.Port, fieldManager string) (*PortApplyConfiguration, error) {
	return extractPort(port, fieldManager, "")
}

// ExtractPortStatus is the same as ExtractPort except
// that it extracts the status subresource applied configuration.
// Experimental!
func ExtractPortStatus(port *apiv1alpha1.Port, fieldManager string) (*PortApplyConfiguration, error) {
	return extractPort(port, fieldManager, "status")
}

func extractPort(port *apiv1alpha1.Port, fieldManager string, subresource string) (*PortApplyConfiguration, error) {
	b := &PortApplyConfiguration{}
	err := managedfields.ExtractInto(port, internal.Parser().Type("com.github.k-orc.openstack-resource-controller.v3.api.v1alpha1.Port"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(port.Name)
	b.WithNamespace(port.Namespace)
