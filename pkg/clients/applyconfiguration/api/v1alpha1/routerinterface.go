...
// RouterInterfaceApplyConfiguration represents a declarative configuration of the RouterInterface type for use
// with apply.
...
// RouterInterface constructs a declarative configuration of the RouterInterface type for use with
// apply.
func RouterInterface(name, namespace string) *RouterInterfaceApplyConfiguration {
	b := &RouterInterfaceApplyConfiguration{}
	b.WithName(name)
	b.WithNamespace(namespace)
	b.WithKind("RouterInterface")
	b.WithAPIVersion("openstack.k-orc.cloud/v1alpha1")
	return b
}

// ExtractRouterInterface extracts the applied configuration owned by fieldManager from
// routerInterface. If no managedFields are found in routerInterface for fieldManager, a
// RouterInterfaceApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// routerInterface must be a unmodified RouterInterface API object that was retrieved from the Kubernetes API.
// ExtractRouterInterface provides a way to perform a extract/modify-in-place/apply workflow.
...
func ExtractRouterInterface(routerInterface *apiv1alpha1.RouterInterface, fieldManager string) (*RouterInterfaceApplyConfiguration, error) {
	return extractRouterInterface(routerInterface, fieldManager, "")
}

// ExtractRouterInterfaceStatus is the same as ExtractRouterInterface except
...
func ExtractRouterInterfaceStatus(routerInterface *apiv1alpha1.RouterInterface, fieldManager string) (*RouterInterfaceApplyConfiguration, error) {
	return extractRouterInterface(routerInterface, fieldManager, "status")
}
...
func extractRouterInterface(routerInterface *apiv1alpha1.RouterInterface, fieldManager string, subresource string) (*RouterInterfaceApplyConfiguration, error) {
	b := &RouterInterfaceApplyConfiguration{}
	err := managedfields.ExtractInto(routerInterface, internal.Parser().Type("com.github.k-orc.openstack-resource-controller.api.v1alpha1.RouterInterface"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(routerInterface.Name)
	b.WithNamespace(routerInterface.Namespace)
...
func (b *RouterInterfaceApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
