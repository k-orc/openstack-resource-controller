...
func ExtractServer(server *apiv1alpha1.Server, fieldManager string) (*ServerApplyConfiguration, error) {
	return extractServer(server, fieldManager, "")
}

// ExtractServerStatus is the same as ExtractServer except
// that it extracts the status subresource applied configuration.
// Experimental!
func ExtractServerStatus(server *apiv1alpha1.Server, fieldManager string) (*ServerApplyConfiguration, error) {
	return extractServer(server, fieldManager, "status")
}

func extractServer(server *apiv1alpha1.Server, fieldManager string, subresource string) (*ServerApplyConfiguration, error) {
	b := &ServerApplyConfiguration{}
	err := managedfields.ExtractInto(server, internal.Parser().Type("com.github.k-orc.openstack-resource-controller.api.v1alpha1.Server"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(server.Name)
	b.WithNamespace(server.Namespace)
...
func (b *ServerApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
