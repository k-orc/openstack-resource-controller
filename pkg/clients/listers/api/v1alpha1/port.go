...
type PortLister interface {
	...
	List(selector labels.Selector) (ret []*apiv1alpha1.Port, err error)
...
type portLister struct {
	listers.ResourceIndexer[*apiv1alpha1.Port]
}

// NewPortLister returns a new PortLister.
func NewPortLister(indexer cache.Indexer) PortLister {
	return &portLister{listers.New[*apiv1alpha1.Port](indexer, apiv1alpha1.Resource("port"))}
}

// Ports returns an object that can list and get Ports.
func (s *portLister) Ports(namespace string) PortNamespaceLister {
	return portNamespaceLister{listers.NewNamespaced[*apiv1alpha1.Port](s.ResourceIndexer, namespace)}
}
...
type PortNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apiv1alpha1.Port, error)
