...
Commit the scaffolding with the command used (skip pre-commit hooks since scaffolding output isn't complete yet):
...
**IMPORTANT**: Use the exact field names from gophercloud (discovered in Step 1):
- If gophercloud has `VipSubnetID` → use `VipSubnetRef` (not just `SubnetRef`)
- If gophercloud has `SourcePortID` → use `SourcePortRef` (not just `PortRef`)
- Preserve all prefixes to match OpenStack API conventions
...
Follow the patterns in @.claude/skills/new-controller/patterns.md when implementing the actuator and API types.
...
Complete the test stubs in `internal/controllers/<kind>/tests/` and run tests following @.claude/skills/testing/SKILL.md
